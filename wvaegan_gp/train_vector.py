...
def sample_image(epoch=None, data_num=12):
    ...
    if epoch is not None:
        save_coords(en_coords*coord_std+coord_mean, labels, "wvaegan_gp/coords/epoch_{0}_dim{1}_vector".format(str(epoch).zfill(3), opt.latent_dim))
    else:
        np.savez("wvaegan_gp/results/final_dim{0}_vector".format(opt.latent_dim), labels, en_coords*coord_std+coord_mean)
        save_coords(en_coords*coord_std+coord_mean, labels, "wvaegan_gp/coords/final_dim{0}_vector.png".format(opt.latent_dim))
...
for epoch in range(opt.n_epochs-done_epoch):
    ...
    for i, (coords, labels) in enumerate(dataloader):
        ...
        if epoch % 5000 == 0:
            torch.save(encoder.state_dict(), "wvaegan_gp/results/encoder_params_dim{0}_vector_{1}".format(opt.latent_dim, epoch))
            torch.save(decoder.state_dict(), "wvaegan_gp/results/decoder_params_dim{0}_vector_{1}".format(opt.latent_dim, epoch))
            torch.save(discriminator.state_dict(), "wvaegan_gp/results/discriminator_params_dim{0}_vector_{1}".format(opt.latent_dim, epoch))
        if epoch % 5000 == 0:
            sample_image(epoch=epoch)

        

torch.save(encoder.state_dict(), "wvaegan_gp/results/encoder_params_dim{0}_vector_{1}".format(opt.latent_dim, opt.n_epochs+done_epoch))
torch.save(decoder.state_dict(), "wvaegan_gp/results/decoder_params_dim{0}_vector_{1}".format(opt.latent_dim, opt.n_epochs+done_epoch))
torch.save(discriminator.state_dict(), "wvaegan_gp/results/discriminator_params_dim{0}_vector_{1}".format(opt.latent_dim, opt.n_epochs+done_epoch))
sample_image(data_num=100)
save_loss(enc_losses, dec_losses, dis_losses, path="wvaegan_gp/results/loss_dim{0}_vector.png".format(opt.latent_dim))