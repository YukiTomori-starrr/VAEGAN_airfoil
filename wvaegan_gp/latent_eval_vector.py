...
class Eval:
  def __init__(self, Enc_PATH, Dec_PATH, coords_npz):
    ...
    self.Dec = Decoder(4).to("cuda")
    self.Dec.load_state_dict(dec_state_dict)
    self.Dec.eval()
    self.Enc = Encoder(4).to("cuda")
  ...
  def visualize_latentspace(self, dim, latent_vector):
    ...
    if dim==3:
      ...
      latent_vecs = TSNE(n_components=3).fit_transform(latent_vector)
...
if __name__ == "__main__":
  ...
  Enc_PATH = "wvaegan_gp/results/encoder_params_dim4_vector_50000"
  Dec_PATH = "wvaegan_gp/results/decoder_params_dim4_vector_50000"
  ...
  labels = to_cuda(Variable(torch.reshape(perfs.float(), (coords.shape[0], opt.n_classes))))
  
  log_variances, mu = evl.Enc(real_imgs, labels, latent_dim=opt.latent_dim)
  variances = torch.exp(log_variances * 0.5)
  
  evl.visualize_latentspace(dim=3, latent_vector=mu)